...
def normalize_mac(mac: str) -> bytes:
    """Konvertuoja MAC adresą į baitus (pvz. 'AA:BB:CC:DD:EE:FF')."""
...
def build_magic_packet(mac: Union[str, bytes]) -> bytes:
    """Sukuria WoL magic packet (102 baitai)."""
...
def send_magic_packet(
    ...
    *,
    ...
    """
    Siunčia Wake-on-LAN magic packet nurodytam MAC adresui.

    Args:
        mac: Tikslinio kompiuterio MAC adresas.
        ip_address: Siuntimo adresas (dažniausiai transliacijos).
        port: UDP prievadas (numatytasis 9).
        interface: Vietinio tinklo sąsajos IP (subnet broadcast siuntimui).
    """
...
def wake(mac: str, **kwargs) -> None:
    """Trumpas alias – įjungia kompiuterį pagal MAC."""
